...
async def parsing(message: types.Message):
    ...
    if message.from_user.id in data_dict:
    ...
    if parser.state != 'ok':
        await message.answer('Ошибка! текст ошибки: ')
        if parser.error == 'Неверный логин или пароль.':
            data_dict.clear()
        await message.answer(f'"{parser.error}"')
    else:
...
async def parsing_login(message: types.Message):
    if ' ' not in message.text:
        await message.answer('Введите корректный логин пароль')
    else:
